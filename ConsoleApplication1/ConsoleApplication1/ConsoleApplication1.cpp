...
int main(int argc, char** argv) {
    ...
    writeMatrix(matrix1, "C:\\Users\\esh20\\OneDrive\\Рабочий стол\\HTML+MATH\\Paral3\\Parprog3\\matrix1.txt");

    std::vector<std::vector<int>> matrix2 = generateRandomMatrix(rows2, cols2);
    writeMatrix(matrix2, "C:\\Users\\esh20\\OneDrive\\Рабочий стол\\HTML+MATH\\Paral3\\Parprog3\\matrix2.txt");

    // Чтение матриц из файлов
    int rows_read1, cols_read1, rows_read2, cols_read2;
    matrix1 = readMatrix("C:\\Users\\esh20\\OneDrive\\Рабочий стол\\HTML+MATH\\Paral3\\Parprog3\\matrix1.txt", rows_read1, cols_read1);
    matrix2 = readMatrix("C:\\Users\\esh20\\OneDrive\\Рабочий стол\\HTML+MATH\\Paral3\\Parprog3\\matrix2.txt", rows_read2, cols_read2);

    const int rowsPerProcess = rows1 / MPI_COMM_SIZE;
    auto start = std::chrono::high_resolution_clock::now();
    std::vector<std::vector<int>> result = parallelMultiplyMatricesFromFile("C:\\Users\\esh20\\OneDrive\\Рабочий стол\\HTML+MATH\\Paral3\\Parprog3\\matrix1.txt", "C:\\Users\\esh20\\OneDrive\\Рабочий стол\\HTML+MATH\\Paral3\\Parprog3\\matrix2.txt", myRank, MPI_COMM_SIZE, rows1, rowsPerProcess);
    auto end = std::chrono::high_resolution_clock::now();
    if (myRank == 0) {
        writeMatrix(result, "C:\\Users\\esh20\\OneDrive\\Рабочий стол\\HTML+MATH\\Paral3\\Parprog3\\result_matrix.txt");
    }
